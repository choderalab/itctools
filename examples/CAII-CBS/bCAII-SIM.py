#! /usr/bin/env python
"""
Script for generation of input files for bovine bCAII:CBS ITC.
...
for ligand, ligand_solution, ligand_ka in zip(ligands, ligand_solutions, ligand_kas):
    ...
    # Define SIM experiments
    for experiment in ligand_protein_experiments:
        experiment = copy.deepcopy(experiment)
        experiment.name = experiment.name + ' SIM'
        experiment.protocol = sim_protocol
        SIM_experiments.append(experiment)

    # TODO, since we are changing ligands, we'd have to wash the syringe.
    # Add ligand to protein experiment(s) to set
    for (ligand_protein_experiment, SIM_experiment) in zip(ligand_protein_experiments, SIM_experiments):
        itc_experiment_set.addExperiment(ligand_protein_experiment)
        itc_experiment_set.addExperiment(SIM_experiment)
