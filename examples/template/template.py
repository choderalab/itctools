#! /usr/bin/env python
"""
Template script for generation of input files for ITC
"""
...
def generate_files(protein_volume,
                   protein_concentration,
                   buffer_masses,
                   compound_masses,
                   cell_concentrations,
                   number_of_itc_plates,
                   nreplicates=1,                             # number of replicates of each protein-ligand experiment
                   cell_volume=202.8,                         # The sample cell volume in microliters
    ...
    solubilities_df = pd.read_csv('../PHIPA/swissadme_logS_53-compounds.csv')
    solubilities_df.set_index('ID', inplace=True)
    purities_df = pd.read_csv('../PHIPA/compound_list.csv')
    ...
    buffer = Solvent('buffer',
                     density=1.014 * ureg.gram / ureg.milliliter)  # TODO is our density the same as the HOST-GUEST buffer?
    ...
    for compound_id in compound_ids:
        ...
        compound = Compound(f'Compound {compound_id}', molecular_weight=molecular_weight, purity=purity,
                            solubility=solubility)
        ligands.append(compound)
        Ka = 1.0 / Kd
        ligand_kas.append(Ka)
        print(
            f"{compound.name:20} : MW {molecular_weight.to('gram/mole')},  purity {purity:.3f}, solubility = {(solubility.to('milligrams/milliliter')):8.3f}, Kd = {((1 / Ka).to('micromolar')):8.3f}")
    print('')
    ...
    for (index, ligand) in enumerate(ligands):
        ...
        ligand_solution = SimpleSolution(compound=ligand,
                                         compound_mass=target_compound_mass,
                                         solvent=buffer,
                                         solvent_mass=target_buffer_mass,
                                         location=PipettingLocation(
    ...
    ligand_solutions = list()
    for (index, ligand) in enumerate(ligands):
        ligand_solution = SimpleSolution(compound=ligand, compound_mass=compound_masses[ligand.name], solvent=buffer,
                                         solvent_mass=buffer_masses[ligand.name], location=PipettingLocation(
                source_plate.RackLabel,
                source_plate.RackType,
                index + 1))
        ligand_solutions.append(ligand_solution)
...
"""
line 24 which compounds to use
line 66 protein_volume
line 67 protein concentration
line 92 amt buffer to mix ligand powder
line 93 mass of compound
line 116 cell concentration - concentration of protein that is being titrated against
line 195 number of ITC plates it can take
"""